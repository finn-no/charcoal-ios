...
extension UIFont {
    static func registerFont(with filenameString: String) {
        if let bundleURL = Bundle(for: FilterKit.self).url(forResource: "FilterKit", withExtension: "bundle") {
        ...
        if let bundleIdentifier = Bundle(for: FilterKit.self).bundleIdentifier {
